...
import { ShieldCheck, History, Activity, Lock, CheckCircle2, FileLock2, Link2, Zap } from 'lucide-react';
  ...
  const doSync = async () => {
    try {
      ...
      if (res.success && res.data) {
        ...
        // Still update sync heartbeat even on empty data
      ...
      <div className="space-y-8 animate-fade-in">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b pb-6">
          ...
          <div className="flex items-center gap-6">
             <div className="hidden lg:flex gap-4 border-r pr-6 print-hide">
                  ...
                  <CardContent className="px-4">
                    <div className="space-y-4">
                      ...
                      {(!auditLogs || auditLogs.length === 0) && (
                        <p className="text-center py-8 text-[11px] text-muted-foreground italic">Awaiting audit events...</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="space-y-4 px-4">
                    <div className="p-4 border border-amber-500/20 rounded-xl bg-card shadow-sm space-y-3">
                      ...
                      <p className="text-[10px] text-muted-foreground leading-relaxed">
                        {mostRecentCritical ? mostRecentCritical.detail : 'Claim CPT mismatch detected against EOC Benefit Schedule. Policy specifies 20% Co-Ins; Bill reflects 40%.'}
                      </p>
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full h-8 text-[9px] font-bold border-amber-500/20 text-amber-600 hover:bg-amber-50"
                        onClick={() => openAppeal(mostRecentCritical?.id)}>
        ...
        <div className="relative h-10 w-full bg-muted/20 border rounded-full overflow-hidden flex items-center px-4 gap-4 print-hide">
           ...
           <div className="flex-1 overflow-hidden relative h-full flex items-center">
             <div className="flex gap-12 animate-marquee whitespace-nowrap text-[10px] font-mono text-muted-foreground uppercase absolute">
                <span>[PREDICTIVE V2.2]: OOP MAX THRESHOLD PROJECTED FOR {new Date().getFullYear()}</span>
                <span>[BRIDGE]: EOC-BILL CONTRADICTION DETECTED IN ACTIVE SESSION</span>
                <span>[SCRUBBER]: ZERO-KNOWLEDGE HMAC VALIDATED</span>
                <span>[SYSTEM]: DISTRIBUTED AUDIT BRIDGE OPERATIONAL</span>
                <span>[COMPLIANCE]: NSA RULES APPLIED TO IN-FLIGHT CLAIMS</span>
             </div>
           </div>
           <div className="hidden sm:flex items-center gap-2 shrink-0">
             <CheckCircle2 className="h-3 w-3 text-emerald-500" />
             <span className="text-[9px] font-bold text-emerald-600 uppercase">HIPAA V2.2 VERIFIED</span>
           </div>
        </div>
      </div>
      <VobChecklist
