...
import { FileText, ClipboardCopy, Printer, ShieldCheck, Sparkles, Loader2, AlertCircle } from 'lucide-react';
      ...
      <DialogContent className="max-w-5xl h-[85vh] flex flex-col p-0 gap-0 overflow-hidden shadow-glass border-blue-500/20">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden bg-background">
          ...
          <div className="w-1/3 border-r p-4 space-y-3 bg-muted/20 overflow-y-auto print-hide">
            ...
            <div className="mt-6 p-4 rounded-xl bg-blue-500/5 border border-blue-500/20">
              ...
              <p className="text-[9px] text-muted-foreground leading-relaxed">
                All templates automatically integrate <span className="text-blue-600 font-bold">ERISA Sec. 502</span> and <span className="text-blue-600 font-bold">NSA</span> legal citations based on detected audit triggers.
              </p>
            </div>
          </div>
          {/* Preview Area */}
          <div className="flex-1 flex flex-col bg-white dark:bg-slate-950">
            ...
            <ScrollArea className="flex-1 p-8 md:p-12">
              <div className="max-w-2xl mx-auto space-y-8 font-serif text-slate-900 dark:text-slate-100 print-only">
        ...
        <DialogFooter className="p-4 border-t bg-muted/10 print-hide">
          <div className="flex items-center gap-4 text-muted-foreground mr-auto">
             ...
             <span className="text-[10px] font-medium italic">Forensic scrubbing will be applied before final print output.</span>
          </div>
          <Button variant="ghost" onClick={close} className="h-9 px-6 font-bold text-xs uppercase">Discard</Button>
          <Button 
            className="h-9 px-6 bg-blue-600 hover:bg-blue-700 font-bold text-xs uppercase shadow-glow"
            onClick={handlePrint}
            disabled={isScrubbing}
          >
            {isScrubbing ? <Loader2 className="h-3 w-3 animate-spin mr-2" /> : <Printer className="h-3 w-3 mr-2" />}
            Export & Print
          </Button>
        </DialogFooter>
      </DialogContent>
